...
def split_ssl_data(args, data, target, num_labels, num_classes, index=None, include_lb_to_ulb=True):
    ...
    num_zeros = np.count_nonzero(lbs)
    # logging.warning('有标签样本中正样本的个数为：{}'.format(num_zeros))
...
def sample_labeled_data(args, data, target,
    ...
    dump_path = os.path.join(args.save_dir, args.save_name,args.num_labels,args.version, 'sampled_label_idx.npy')
