...
def get_transform(mean, std, crop_size, train=True): # weakly augmented
    if train:
        return transforms.Compose([transforms.ToTensor(), #transforms.RandomHorizontalFlip(),transforms.RandomVerticalFlip(),
...
class SSL_Dataset:
    ...
    def get_data(self, svhn_extra=True):
        ...
        shape of labels: B,
        """
        if 'HTRU' in self.name.upper():
            folder = '/aidata/Ly61/number3/HTRU-FAST-sub/'
            # FAST_folder = '/aidata/Ly61/number3/FAST/'
            # HTRU_folder = '/aidata/Ly61/number3/HTRU/'
            # folder = HTRU_folder
            # folder = FAST_folder
            if self.train == True:  
                with open(os.path.join(folder,'data_batch_train'),'rb') as f:
                    img_dict=pickle.load(f,encoding='bytes')
                    #print(img_dict.keys())
                    data,targets = img_dict['data'],img_dict['labels']
                    data = np.vstack(data).reshape(-1, 3, 32, 32)
                    data = data.transpose((0, 2, 3, 1))
                # with open(os.path.join(FAST_folder,'data_batch_train'),'rb') as FAST_f:
                #     FAST_img_dict=pickle.load(FAST_f,encoding='bytes')
                #     FAST_data,FAST_targets = FAST_img_dict['data'],FAST_img_dict['labels'] 
                #     FAST_data = np.vstack(FAST_data).reshape(-1, 3, 32, 32)
                #     FAST_data = FAST_data.transpose((0, 2, 3, 1)) 
                #     data = np.concatenate((FAST_data, data), axis=0) 
                #     # print('target:',np.array(targets).shape,np.array(FAST_targets).shape) 
                #     targets = FAST_targets + targets  
                    # print('target:',np.array(targets).shape,np.array(FAST_targets).shape) 
                    
                    #print(np.array(data).shape,np.array(targets).shape)
            elif self.train == False:  
                with open(os.path.join(folder,'data_batch_val'),'rb') as f:
                    img_dict=pickle.load(f,encoding='bytes')
                    data,targets = img_dict['data'][:1000],img_dict['labels'][:1000]
                    data = np.vstack(data).reshape(-1, 3, 32, 32)
                    data = data.transpose((0, 2, 3, 1)) 
                # with open(os.path.join(FAST_folder,'data_batch_val'),'rb') as FAST_f:
                #     FAST_img_dict=pickle.load(FAST_f,encoding='bytes')
                #     FAST_data,FAST_targets = FAST_img_dict['data'],FAST_img_dict['labels'] 
                #     FAST_data = np.vstack(FAST_data).reshape(-1, 3, 32, 32)
                #     FAST_data = FAST_data.transpose((0, 2, 3, 1)) 
            ...
            if self.args.test == True:  
                with open(os.path.join(folder,'data_batch_test'),'rb') as f:
                    img_dict=pickle.load(f,encoding='bytes')
                    # data,targets = img_dict['data'][:1373*2],img_dict['labels'][:1373*2]
                    # 前1373个样本
                    data_first_part = img_dict['data'][:1373]
                    labels_first_part = img_dict['labels'][:1373]

                    # 获取数据的长度
                    n_samples = len(img_dict['data'])

                    # 生成随机索引。这里我们从1373开始到数据末尾的索引，然后选择1373个样本
                    random_indices = np.random.choice(range(1373, n_samples), size=1373, replace=False)

                    # 使用随机索引选取数据和标签的第二部分
                    data_second_part = img_dict['data'][random_indices]
                    labels_second_part = np.array(img_dict['labels'])[random_indices]
                    

                    # 将两部分数据和标签合并
                    data = np.concatenate((data_first_part, data_second_part), axis=0)
                    targets = np.concatenate((labels_first_part, labels_second_part), axis=0)
                    data = np.vstack(data).reshape(-1, 3, 32, 32)
                    data = data.transpose((0, 2, 3, 1))   
                # with open(os.path.join(FAST_folder,'data_batch_test'),'rb') as FAST_f:
                #     FAST_img_dict=pickle.load(FAST_f,encoding='bytes')
                #     FAST_data,FAST_targets = FAST_img_dict['data'],FAST_img_dict['labels'] 
                #     FAST_data = np.vstack(FAST_data).reshape(-1, 3, 32, 32)
                #     FAST_data = FAST_data.transpose((0, 2, 3, 1)) 
    ...
    def get_dset(self, is_ulb=False,
        ...
        # data_VAE,targets_VAE = self.get_VAE_data()
        # data = np.concatenate((data,data_VAE[:18000]), axis=0) 
        # targets = np.concatenate((targets,targets_VAE[:18000]), axis=0)
        num_classes = self.num_classes
        transform = self.transform

        return BasicDataset(self.args,self.alg, data, targets, num_classes, transform,
                            is_ulb, strong_transform, onehot)
                            
    def get_VAE_data(self):
        folder = '/aidata/Ly61/number3/HTRU-sub/'
        with open(os.path.join(folder,'data_batch_VQVAE'),'rb') as f:
            img_dict=pickle.load(f,encoding='bytes')
            #print(img_dict.keys())
            data,targets = img_dict['data'][:45000],img_dict['labels'][:45000]
            data = np.vstack(data).reshape(-1, 3, 32, 32)
            data = data.transpose((0, 2, 3, 1))
        return data,targets
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        if self.alg == 'fullysupervised':
            ...
            data_VAE,targets_VAE = self.get_VAE_data()
            lb_data = np.concatenate((lb_data,data_VAE), axis=0) 
            #print(ulb_data.shape)
            lb_targets = np.concatenate((lb_targets,targets_VAE), axis=0)
            lb_dset = BasicDataset(self.args,self.alg, lb_data, lb_targets, self.num_classes,
        ...
        if self.name.upper() == 'STL10':
            ...
            ulb_targets = None
        elif 'HTRU' in self.name.upper():
            data, targets = self.get_data()
            lb_data, lb_targets, ulb_data, ulb_targets = split_ssl_data(self.args, data, targets,
                                                                        num_labels, self.num_classes,
                                                                        index, include_lb_to_ulb)   # 将HTRU数据集分为有标签和无标签
            data_VAE,targets_VAE = self.get_VAE_data()
            #print(ulb_data.shape)
            #print(data_VAE.shape)
            #print(targets_VAE[:10])
            # lb_data = np.concatenate((ulb_data,data_VAE), axis=0) 
            #print(ulb_data.shape)
            # lb_targets = np.concatenate((ulb_targets,targets_VAE), axis=0)
            ulb_data = np.concatenate((ulb_data,data_VAE), axis=0) 
            #print(ulb_data.shape)
            ulb_targets = np.concatenate((ulb_targets,targets_VAE), axis=0)
                                                                        
        else:
            data, targets = self.get_data()
            lb_data, lb_targets, ulb_data, ulb_targets = split_ssl_data(self.args, data, targets,
                                                                        num_labels, self.num_classes,
                                                                        index, include_lb_to_ulb)   # 将cifar等类型的数据集分为有标签和无标签
        ...
        ulb_dset = BasicDataset(self.args,self.alg, ulb_data, ulb_targets, self.num_classes,
