    ...
    def __init__(self,
        ...
        if self.is_ulb:
            if strong_transform is None:
                # self.strong_transform = VQVAE(args).cuda(args.gpu)
                # self.strong_transform.load_checkpoint('/aidata/Ly61/number3/python/vqvae_gpt/HTRU_vqvae_checkpoints_4/vqvae_epoch_best.pt')
                # self.strong_transform.eval()
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is None:
            ...
            if isinstance(img, np.ndarray):
                ...
                # tensor_img = torch.from_numpy(imgs.astype(np.float32).reshape(1,-1,32,32)).cuda(self.args.gpu)
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'fixmatch':
                    # re_imgs,_,_ = self.strong_transform(tensor_img)
                    # re_imgs = re_imgs.cpu().squeeze()
